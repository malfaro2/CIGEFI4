...
####################### TEMPERATURE INDEX #################################
###### 5 groups of months: DJF, MA, MJ, JA, SO ###########################
dat <- list()
for(i in 1:5){
  dat[[i]]<- readMat(paste0("data_original/index_bimonth_temp/periodo",i,".mat"))
}

latlontemp <- readMat("data_original/index_bimonth_temp/latlon.mat")
temp <- dat

# 38 stations, 1970-2004, 10 index
# 32 years x 5 groups of months

length(temp)#bimonths
lapply(temp, length)#index for each bimonth
...
dat <- list()
for(i in 1:5){
  dat[[i]]<- readMat(paste0("data_original/index_bimonth_prec/periodo",i,".mat"))
}

latlonprec <- readMat("data_original/index_bimonth_prec/latlon.mat")
prec <- dat
# 174 stations, 1979-2010, 10 index
# 32 years x 5 groups of months

length(prec)#bimonths
lapply(prec, length)##index for each bimonth
...
############################ CREATE TABLES ################################
## Create a tibble with all variables: 32*5*174 (years,bimonths,stations)
